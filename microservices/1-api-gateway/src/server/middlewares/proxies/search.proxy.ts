...
//     target: config.SEARCH_BASE_URL,
//     changeOrigin: true,
// });


export function searchProxyMiddleware(req: Request, res: Response, next: NextFunction) {
    const redisConnection = RedisConnection.getInstance();
    const redisCache = new RedisCache(redisConnection);
    const cacheInfo = req.cacheInfo;

    const searchCachingProxy = createProxyMiddleware({
        target: config.SEARCH_BASE_URL,
        changeOrigin: true,
    ...
    searchCachingProxy(req, res, next);
}